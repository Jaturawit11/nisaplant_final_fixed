...
function SellPageInner() {
  ...
  useEffect(() => {
    if (payStatus === 'partial' || payStatus === 'paid') {
  ...
  const totals = useMemo(() => {
    ...
    return { totalCost, totalPrice, profit, avgProfit }
  }, [items])
    ...
    <AppShell title="ขาย">
      <div className="-m-3 min-h-full rounded-[34px] bg-[linear-gradient(180deg,#fffdfd_0%,#fff8fb_24%,#f7fbff_58%,#f8fff9_100%)] p-3 sm:-m-4 sm:p-4 md:-m-5 md:p-5">
        <div className="mx-auto grid max-w-6xl gap-3 sm:gap-4">
          ...
          <ShellCard
            title="ข้อมูลบิล"
            subtitle="กรอกเท่าที่จำเป็นก่อน ที่เหลือค่อยเปิดเพิ่ม"
            tint="default"
            right={
              <button
                type="button"
              ...
              >
                {showAdvanced ? 'ซ่อนรายละเอียด' : 'เปิดตัวเลือกเพิ่มเติม'}
              </button>
            }
          >
          ...
          <div className="grid gap-3 sm:gap-4 xl:grid-cols-[1.18fr_0.82fr]">
            <ShellCard
              ...
              subtitle="วางรหัสทีละตัว หลายตัว หลายบรรทัด หรือช่วงรหัสได้ในช่องเดียว"
              tint="rose"
              right={
                <Link href="/stock" className={ghostBtnClass}>
                  เปิดคลังไม้
                </Link>
              }
            >
              <div className="rounded-[22px] border border-white/85 bg-white/72 p-4 text-xs leading-6 text-slate-500">
                ตัวอย่าง: <span className="font-semibold text-slate-700">N-2603-0002, 26030002, N-2603-0005 ถึง N-2603-0015</span>
              </div>
              ...
              {previewRows.length ? (
                <div className="mt-4 grid gap-2">
                  ...
                  {previewRows.length > 24 ? (
                    <div className="pt-1 text-xs text-slate-400">แสดง 24 รายการแรก</div>
                  ) : null}
                </div>
              ) : null}
            </ShellCard>
